...
router.post("/", authMiddleware, async (req, res) => {
  const { text, location, picUrl, isTravel, mapData, distance } = req.body;
  ...
  try {
    ...
    if (isTravel) newPost.isTravel = isTravel;
    if (mapData) newPost.mapData = mapData;
    if (distance) newPost.distance = distance
...
router.get("/", authMiddleware, async (req, res) => {
  const { pageNumber, isTravel } = req.query;
  try {
    ...
    if (number === 1) {
        posts = await PostModel.find({isTravel})
          .limit(size)
          .sort({ createdAt: -1 })
          .populate("user")
...
router.get("/api/posts:postId", authMiddleware, async (req, res) => {
  try {
...
router.post("/join/:postId", authMiddleware, async (req, res) => {
  try {
    ...
    const isJoined =
      post.joinedUsers.find(joinedUser => joinedUser.user.toString() === userId);

    if (isJoined) {
      return res.status(401).send("USER ALREADY JOINED");
    }

    await post.joinedUsers.unshift({ user: userId });
    await post.save();
    ...
    return res.status(200).send("USER JOIND THE TRIP");
  } catch (error) {
    console.error(error);
    return res.status(500).send(`Server error`);
  }
...
router.put("/unjoin/:postId", authMiddleware, async (req, res) => {
  try {
    const { postId } = req.params;
    const { userId } = req;
    const post = await PostModel.findById(postId);
    ...
    const index = post.joinedUsers.map(like => like.user.toString()).indexOf(userId);

    await post.joinedUsers.splice(index, 1);
